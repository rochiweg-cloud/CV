...
function renderProfileCard() {
  ...
  // Gmail compose URL en lugar de mailto: (que abre Outlook por defecto en Windows)
  const gmailCompose = `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(DATA.email)}`;
  // CV en el idioma activo
  const cvUrl = typeof DATA.cvFile === 'object' ? DATA.cvFile[currentLang] : DATA.cvFile;
  return `
    <div class="profile-card">
      ...
      <div class="contact-bar">
        <a href="${gmailCompose}" target="_blank" rel="noopener" title="${DATA.email}">${ICONS.mail}<span>${DATA.email}</span></a>
      ...
      <div class="cta-buttons">
        <a href="${cvUrl}" download class="btn btn-primary">${ICONS.download} ${t(DATA.ui.downloadCV)}</a>
      </div>
    </div>
  `;
}
  ...
  return `
    <header class="site-header">
      <nav class="nav-container">
        <ul class="nav-links">
          <li><a href="index.html" class="${activePage === 'about' ? 'active' : ''}">${t(DATA.ui.nav.about)}</a></li>
          <li><a href="projects.html" class="${activePage === 'projects' ? 'active' : ''}">${t(DATA.ui.nav.projects)}</a></li>
        </ul>
